...
- Ran first try (python3→pyfaidx fix carried it): 1,992 orthogroups from **projection edges alone**. The rbest + graph edge sources contributed **0** — resolved in #30 below.

30. **Phase E edge sources were non-functional stubs** (the 3-source consensus silently ran as 1-source). Three layers: (a) `06_consensus.sh` reads `inputs/annotations/*.bed` but **nothing generates them** → both rbest and graph got empty input → 0 edges. (b) Once BEDs exist, `phase_e_rbest_overlap.py` was a **stub** that took the cartesian product of all genes under a chain's whole-chromosome *header span* → **11 M garbage edges** (would collapse the union-find). (c) `phase_e_graph_edges.py` keys co-membership by *path name*, but `odgi paths --haplotypes` emits one path per single strain (PanSN) → "multi-strain paths" is always empty → 0; it never uses the node-membership matrix. **Fixes:** added a BED-from-GFF step to `06_consensus.sh` (self-contained); **rewrote `phase_e_rbest_overlap.py` to do block-level projection** — project each target gene through the chain's aligned blocks and connect it to its single best-overlapping query gene (reciprocal ≥0.90). Result: rbest now yields **7,778 correct reciprocal-best edges**; with projection the consensus consolidates from 1,992 → **1,367 orthogroups** (2 of 3 sources working). **Still open:** the graph (node-level co-membership) source — needs mapping each gene's coords to graph node IDs per strain (`odgi position`) then intersecting node sets across strains; left as a documented stub (0 edges, harmless to union-find).
...
29. **`verify_essentials.sh` carried the same validator bugs as the phase scripts** (it had never been run either): `samtools quickcheck` on FASTA (and bare `samtools`, absent on host) → check by faidx index; AXT first-9-lines `awk` → `grep -qE '^[0-9]+ '`; cohort-VCF check used an **undefined `cmd`** wrapper and a `zcat|grep` that trips pipefail (SIGPIPE) → check the `.csi` index exists. After these, **`verify_essentials.sh` exits 0: ALL ESSENTIALS VERIFIED, 98 checks pass.**

## ✅ FULL PIPELINE GREEN + VERIFIED (A → J) on the Pv4 test panel

First complete end-to-end run of the clean scaffold — `verify_essentials.sh` passes (98 OK, 0 missing). **29 fixes** across all phases. Outputs: mash matrix + BUSCO; 5 soft-masked genomes; 56 chains; 12 projected annotations; PGGB graph; 1,992 orthogroups; 509 strict + 810 relaxed codon MSAs (trimal-cleaned); 275+514 trees; 275+514 BUSTED JSONs; 5 multiz MAFs; 4 projected cohort VCFs. Phase K (UCSC hub) is a separate publishing step (Pv4's hub already built).
