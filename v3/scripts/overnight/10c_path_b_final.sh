#!/bin/bash
# Path B cohort build — final version. Fixes the header-injection bug.
# - Ensures all 16 per-chr .vcf files exist (rewriting if needed)
# - Injects target contig ##contig= lines into each header
# - Sort + bgzip + tabix per-chr
# - Concat
...
log() { echo "$(date +%H:%M:%S) [path_b_final] $1" | tee -a logs/overnight/STATUS.md; }
...
build_one() {
  ...
  local COHORT=projection/B_graph/Pv4_cohort_on_${O}.vcf.gz

  # Skip target if cohort already exists AND is reasonable size
  if [ -s $COHORT ] && [ $(stat -c '%s' $COHORT) -gt 100000000 ]; then
    log "$O: cohort already exists ($(du -h $COHORT | cut -f1)), skipping"
    return 0
  fi
  # Clean tiny broken cohort
  [ -s $COHORT ] && [ $(stat -c '%s' $COHORT) -lt 1000000 ] && rm -f $COHORT ${COHORT}.csi
  log "$O START"

  # Python rewrite — only for missing per-chr .vcf or .vcf.gz
  python3 - "$O" "$TSV" "$OUT" "$HDR_DIR/${O}.contigs" <<'PYEOF'
import csv, gzip, os, sys, glob
O, TSV, OUT, HDR = sys.argv[1:5]

# Build header snippet contents
with open(HDR) as f:
    contig_lines = f.read()
        ...
        for ln in fi:
            if ln.startswith('#'):
                if ln.startswith('##contig='):
                    continue  # drop source contigs
                if ln.startswith('##fileformat='):
                    fo.write(ln)
                    fo.write(contig_lines)  # inject target contig declarations
                    continue
                fo.write(ln)
                continue
            n_in += 1
...
PYEOF
  ...
  # Sort each .vcf → .vcf.gz; only delete .vcf after .vcf.gz confirmed
  ...
  for vcf in $OUT/Pv4_*.vcf; do
    ...
    if docker run --rm -v $V3:$V3 -v $SCRATCH:$SCRATCH -w $V3 \
        ...
        bcftools sort -T $SORT_TMP -Oz -o $tmp_gz $vcf 2>>logs/overnight/path_b_final_${O}.log; then
      mv $tmp_gz $gz
      docker run --rm -v $V3:$V3 -w $V3 \
        quay.io/biocontainers/bcftools:1.20--h8b25389_0 \
        tabix -p vcf $gz 2>>logs/overnight/path_b_final_${O}.log
      if [ -s $gz ]; then
        rm -f $vcf
        n_done=$((n_done+1))
      else
  ...
  if [ -z "$FILES" ]; then log "$O: no concat inputs"; return 1; fi
  docker run --rm -v $V3:$V3 -w $V3 \
    quay.io/biocontainers/bcftools:1.20--h8b25389_0 \
    bash -c "bcftools concat -a $FILES -Oz -o $V3/$COHORT && bcftools index $V3/$COHORT" \
    2>>logs/overnight/path_b_final_${O}.log
  if [ -s $COHORT ]; then
...
export -f build_one
export V3 SCRATCH SORT_TMP HDR_DIR
